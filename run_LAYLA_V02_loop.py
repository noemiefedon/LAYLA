...
LAYLA_loop is a script applying the optimiser LAYLA to laminates of different
ply counts but the same target lamination parameters.
"""
__version__ = '2.0'
...
from src.LAYLA_V02.save_set_up import save_parameters_LAYLA
...
# Targets
#==============================================================================
ss_target = np.array([60, -60], dtype=int)
lampam_target  = calc_lampam(ss_target)

# Ply counts
beginning = 20 # Minimum laminate ply count
ending = 300 # Maximum laminate ply count
step = 1
...
A = A_from_lampam(lampam_target, mat_prop)
A11_target = A[0, 0]
A22_target = A[1, 1]
A12_target = A[0, 1]
A66_target = A[2, 2]
...
B = B_from_lampam(lampam_target, mat_prop)
B11_target = B[0, 0]
B22_target = B[1, 1]
B12_target = B[0, 1]
B66_target = B[2, 2]
...
D = D_from_lampam(lampam_target, mat_prop)
D11_target = D[0, 0]
D22_target = D[1, 1]
D12_target = D[0, 1]
D66_target = D[2, 2]
...
result_filename = 'WV60-AD-S_' + constraints_set + end_filename + '.xlsx'
...
# How many optimisations to run?
if (ending - beginning) % step == 0:
    l = 1 + ((ending - beginning) // step)
else:
    l = 2 + ((ending - beginning) // step)

# Initialisation of the array storing the results
table_result = pd.DataFrame()

n_plies_lam = beginning # Laminate total number of plies
i = 1 # index

while n_plies_lam < ending + step:
