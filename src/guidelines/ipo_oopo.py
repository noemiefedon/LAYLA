...
- calc_penalty_ipo_oopo_mp
    calculates penalties for in-plane orthotropy and out-of-plane orthotropy
    based on lamination parameters for a multi-panel structure
...
def calc_penalty_ipo_oopo_mp( lampam, constraints):
    """
    calculates penalties for in-plane orthotropy and out-of-plane orthotropy
    based on lamination parameters for a multi-panel structure
    ...
    return penalties_ipo, penalties_oopo

def calc_penalty_ipo_oopo_ss(
        lampam,
        constraints,
        parameters,
        cummul_areas=1,
    ...
    if lampam.ndim == 2:
        n_ss = lampam.shape[0]
        penalties_ipo = np.zeros((n_ss,), dtype=float)
        penalties_oopo = np.zeros((n_ss,), dtype=float)
        if not parameters.opti_problem == 'Lamination parameters matching':
            # penalty for in-plane orthotropy
            if constraints.ipo and  parameters.penalty_ipo_switch:
                for ind_ss in range(n_ss):
                    penalties_ipo[ind_ss] = (
                        abs(lampam[ind_ss][2]) + abs(lampam[ind_ss][3])) / 2
            # penalty for out-of-plane orthotropy
            if constraints.oopo:
                for ind_ss in range(n_ss):
...
def calc_penalty_oopo_ss(
    ...
    retrieved so far + the current plies
    """
    if calculate_penalty or obj_func_param is None \
    or obj_func_param.opti_problem != 'Lamination parameters matching':
...
if __name__ == "__main__":
    ...
    mat = Material()
    print(calc_penalty_ipo_oopo_ss(
        lampam,
