# -*- coding: utf-8 -*-
"""
inner loop optimiser for symmetric laminates
"""
__version__ = '2.0'
...
def outer_step_sym(
    ...
    '''
    inner loop optimiser for S-laminates (symmetric laminates)
...
#            outer_step_result.n_obj_func_calls += result.n_obj_func_calls
    ...
    obj_no_const = objectives(
        lampam=outer_step_result.lampam_best,
        targets=targets,
    ...
    n_plies_per_angle = np.zeros((constraints.n_set_of_angles,), float)
    for ind in range(result.ss_best.size):
        index = constraints.ind_angles_dict[result.ss_best[ind]]
